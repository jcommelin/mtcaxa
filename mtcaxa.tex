...
\begin{proposition} % {{{-2
 ...
 \begin{proof}
  ...
  The other types require a bit more bookkeeping.
  We begin by proving that
  there is an element $\phi \in \Isom_f(\Delta_1,\Delta_2)$,
  that may or may not map $\mu_1$ to~$\mu_2$.
  By \cref{deg2-set} it suffices to show that
  there is a $\Gal(\QQbar/\QQ)$-equivariant isomorphism
  $U(\Delta_1^{\mu_1}) \to U(\Delta_2^{\mu_2})$ lying above~$f$.
  Since the degree of~$U(\Delta_i^{\mu_i})$ over~$\pi_0(\Delta_i)$
  is at most~$2$,
  the existence of such a global isomorphism
  will follow from Chebotarev's density theorem
  if we prove that these sets are locally isomorphic.
  Once again, if $\Delta_1^{\mu_1}$ and~$\Delta_2^{\mu_2}$
  are of type~$A_n$, or~$D_n^\RR$ ($n \ge 5$),
  then the topology of the diagrams
  forces that $\psi_\ell(U(\Delta_1^{\mu_1})) = U(\Delta_2^{\mu_2})$
  for all prime numbers~$\ell$.
  %TODO
