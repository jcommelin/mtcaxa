...
\begin{definition} % {{{-2
 ...
 is the pro-algebraic affine group scheme
 $\iAut^\otimes(\HB)$ over~$\QQ$
...
\begin{notation} % {{{-2
 ...
 Let $K$,~$X$, $x$, $E$, $\lambda$, and~$\rho$
 be as in \cref{unramified-representation}
 and assume that $K$ is a finitely generated field.
...
\begin{definition} % {{{-2
 ...
 Dynkin diagram associated with~$\Delta$.
 (See \cref{extended_dynkin_diagrams}.)
...
\begin{figure} % {{{-2
 ...
 \begin{tikzpicture}[Dynkin] % {{{-3
  ...
  \end{scope}
 \end{tikzpicture} % -}}}3
\caption{Extended Dynkin diagrams.
The ``extending'' node~$\alpha_0$ is depicted by a grey node
\protect\tikz[Dynkin] \protect\node[a0] {};.}
\label{extended_dynkin_diagrams}
\end{figure}
...
(Compared to table~1.3.9 of~\cite{Del_ShimVar}
we have added a diagram~$\DtA_n$
that depicts a special case of diagram~$\DtA_{p+q+1}$,
for reasons of clarity.
See also \S10 of~\cite{Mi13}, in particular footnote~34.)
...
\begin{example} % {{{-2
 ...
 will play an important role in the proofs in the remainder of this section
...
\begin{lemma} % {{{-2
 ...
 $\Aut_\id(\Delta,\mu)^{\Gamma_\QQ} \to
 \Aut_\id(\Delta_\lambda,\mu_\lambda)^{\Gamma_\QQ}$
 is an isomorphism.
 \begin{proof}
...
\begin{itemize}
 \item If $(\Delta,\mu)$ has type~$\DtA_n$,
  we let $U(\Delta,\mu)$ be the set of extremal nodes in~$\Delta$,
  which has degree~$1$ if $n = 1$ and degree~$2$ otherwise.
  (N.b.: This set is equal to~$S$, unless all nodes in~$\mu$ are extremal,
  \emph{cf.}, \cref{table-deligne-dynkin-diagrams}.)
...
\begin{proposition} % {{{-2
 ...
 $\psi_\ell \in \Isom_f\big((\Delta_1,\mu_1)_{\QQl},
 (\Delta_2,\mu_2)_{\QQl}\big)^{\Gamma_{\QQl}}$.
...
\paragraph{} % {{{-2 Start proof of deldyn-local-global
Unfortunately, the proof of this \namecref{deldyn-local-global}
is rather long and technical.
We will first look at some easy cases that give a good idea of the strategy.
After that, the remaining cases are dealt with.
The proof concludes in~\cref{deldyn-local-global-endproof}.

Note that it suffices to prove the existence of~$\phi$;
...
This map~$\phi$ sends $\mu_1$ to~$\mu_2$,
...
is at most~$2$,
the existence of a global isomorphism
...
\begin{itemize}
 ...
 \item Finally, assume that $(\Delta_1,\mu_1)$ and~$(\Delta_2,\mu_2)$
  ...
  Without loss of generality we may assume $\alpha \in \mu_1$.
  By Chebotarev's density theorem,
...
\begin{itemize}
 ...
 \item if the type is $\DtD_4^\RR$,
  ...
  to $U(\Delta_2,\mu_2) = S_2$ by construction; and
...
\paragraph{} % {{{-2 End proof of deldyn-local-global
\label{deldyn-local-global-endproof}
...
\begin{remark} % {{{-2
 In the preceding \namecref{deldyn-local-global},
 the hypothesis that $\psi_\ell$ exists for \emph{each} prime number~$\ell$,
 may be weakened to the condition that $\psi_\ell$ exists
 for a set of prime numbers with Dirichlet density~$1$.
 This weaker condition is sufficient to apply Chebotarev's density theorem
 in~\cref{using-cheb}.
\end{remark}

\section{Deligne--Dynkin diagrams and \tops{$p$}{\it p}-adic Hodge theory} % {{{-1

\begin{readme} % {{{-2
 Let $M$ be a hyperadjoint abelian motive
 over a finitely generated field $K \subset \CC$,
 and let $(\Delta,\mu)$ be the Deligne--Dynkin diagram of~$M$.
 In this short section we apply $p$-adic Hodge theory
 to determine the subset $\mu \subset \Delta$
 from the $p$-adic realisation~$\Hp(M)$.
\end{readme}

\begin{theorem} % {{{-2
 \label{faltings-p-adic-comparison}
 Let $M$ be a motive over a $p$-adic field~$K_v$.
 Let $C_v$ be the completion of an algebraic closure of~$K_v$,
 and for $i \in \ZZ$, write $C_v(i)$ for $C_v \otimes_{\QQp} \QQp(i)$
 with the diagonal action of~$\Gamma_{K_v}$.
 Then there is a functorial isomorphism of $C_v$-vector spaces
 \[
  \Hp(M) \otimes_{\QQp} C_v \cong
  \bigoplus_{i \in \ZZ} \gr^i \HdR(M) \otimes_{K_v} C_v(-i)
 \]
 that is compatible with the action of~$\Gamma_{K_v}$.
 \begin{proof}
  If $M = \HH^n(X)$ for some smooth projective variety~$X$ over~$K_v$,
  then this result is theorem~4.1 of~\cite{Fa88}.
  Since the cited theorem shows that the comparison isomorphism
  is an isomorphism of Weil cohomology theories,
  one deduces that the isomorphism is compatible with motivated cycles.
  This implies that the result is true for a general motive~$M$.
 \end{proof}
\end{theorem}

\begin{corollary} % {{{-2
 Let $\BHT{v}$ denote the graded ring $\bigoplus_{i \in \ZZ} C_v(i)$,
 and let $\FHT{v}$ be the field of fractions of~$\BHT{v}$.
 Then there is a functorial isomorphism of graded $\FHT{v}$-vector spaces
 \[
  (\Hp(M) \otimes_{\QQp} \BHT{v}) \otimes_{\BHT{v}} \FHT{v} \cong
  \gr\HdR(M) \otimes_{K_v} \FHT{v}
 \]
 that is compatible with the action of~$\Gamma_{K_v}$.
 (The grading on the left-hand-side comes from the grading on~$\BHT{v}$.)
\end{corollary}
...
The choices $K \subset K_v \subset C_v$
determine a grading on~$\Hp(M) \otimes_{\QQp} \FHT{v}$.
This grading induces a cocharacter~$\mu_\HT$ of
$\Gp(M) \otimes_{\QQp} \FHT{v} \subset \GG(M) \otimes_\QQ \FHT{v}$
that is called the \emph{Hodge--Tate} cocharacter.
If $M$ is abelian,
then we may view $\mu_\HT$ as cocharacter of $\GB(M) \otimes_\QQ \FHT{v}$ since
$\Gpc(M) \subset \GB(M) \otimes_\QQ \QQp = \GG(M)^\circ \otimes_\QQ \QQp$
by \cref{hodge-is-motivated}.
...
\begin{lemma} % {{{-2
 ...
 \begin{proof}
  ...
  the two cocharacters $\mu_\HT$ and~$\mu$ correspond to two tensor functors,
  namely the realisation functors
  \[
   \begin{aligned}
  ...
  Let $\bar Q \subset \bar Q'$ be an extension of
  algebraically closed fields of characteristic~$0$,
  and let $G$ be an algebraic group over~$\bar Q$.
  Then the natural map $\Cochar(G) \to \Cochar(G_{\bar Q'})$
  obtained by base change, induces a bijection
  between the conjugacy classes of cocharacters of~$G$
  and the conjugacy classes of cocharacters of~$G_{\bar Q'}$.

  Indeed, let $T \subset B \subset G$ be a choice of
  a maximal torus~$T$ and a Borel subgroup~$B$,
  and let $\Fund(T,B) \subset \Cochar(T)$ denote the fundamental chamber.
  Every conjugacy class of cocharacters of~$G$
  has a unique representative in $\Fund(T,B)$,
  and so we find a chain of isomorphisms
  \[
   \Cochar(G)/G \cong \Fund(T,B) \cong \Fund(T_{\bar Q'},B_{\bar Q'})
   \cong \Cochar(G_{\bar Q'})/G_{\bar Q'}.
  \]
  This finishes the proof of the \namecref{hodge-tate-cochar-deldyn}.
 \end{proof}
\end{lemma}
...
Since $M$ is hyperadjoint, the group~$\GG(M)$ is adjoint;
and because $M$ is irreducible, it is of the form $\Res_{E/\QQ}\mathcal{G}$,
where $E$ is a number field,
and $\mathcal{G}$ is an absolutely simple adjoint group over~$E$.
This number field~$E$ is a totally real field
(see the discussion in~\S2.3.4(a) of~\cite{Del_ShimVar}).
We have $E = \End(M)$, once again because
$M$ is hyperadjoint---and therefore corresponds to the
adjoint representation of $\GG(M)$ under the Tannakian formalism.
...
(see corollaire~2.3.3 of~\cite{Del_ShimVar} for details).
...
Recall that $\GB(M) = \Res_{E_i/\QQ}(G_i)$,
for some absolutely simple adjoint group~$G_i$ over~$E_i$.
Since $\MTC(M_i)$ holds
...
\Res_{E_{i,\lambda}/\QQl} (G_i \otimes_{E_i} E_{i,\lambda}).
\]
In particular we have $\Gl(\Hlambda(M_i))
= \Res_{E_{i,\lambda}/\QQl} (G_i \otimes_{E,i} E_{i,\lambda})$
for every $\lambda \in \Lambda_{i,\ell}$.
If we write $\Glambda(M_i)$ for $\Glambda(\Hlambda(M_i))$
then the above computation implies that
$\Glambda(M_i)$ is equal to $G_i \otimes_{E_i} E_{i,\lambda}$,
...
\[
 \begin{tikzcd}
  ...
  && \Glambda(M_i)(E_\lambda) \ar[r,hook] & \GL(\Hlambda(M_i))(E_\lambda)
 \end{tikzcd}
\]
...
\[
 \begin{tikzcd}
  && W_{F,\lambda}^* \ar[d] \\
  && \Glambda'(E_\lambda) \ar[r,hook] \ar[dd,two heads]
  & \GL(\Hlambda')(E_\lambda) \\
  \Gamma_K \ar[drr,bend right,"\rho_\lambda"] &
  \widetilde{\Glambda}(E_\lambda) \ar[dr,two heads] \ar[ur] \\
  && \Glambda(E_\lambda) \ar[r,hook] & \GL(\Hlambda)(E_\lambda)
 \end{tikzcd}
\]
