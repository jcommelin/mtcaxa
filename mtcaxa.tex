...
Let $\Delta$ be the Dynkin diagram of~$\GB(M)$.
...
(If $M$ is non-trivial, then $\mu$ is non-empty.)
By \S1.2.5 of~\cite{Del_ShimVar} the nodes in~$\mu$ are special,
and hence $\Delta^\mu$ is a populated Deligne--Dynkin diagram over~$\QQ$
that we call the \emph{Deligne--Dynkin diagram associated with $M$}.
...
This involution depends only on the type of~$\Delta$.
