...
{\narrower
 ...
 \medskip\noindent
 Let $X$ be a smooth projective variety over a number field $K \subset \CC$.
 The Mumford--Tate conjecture is a precise way of saying that
 the extra structure on the singular cohomology groups of~$X$
 (a Hodge structure)
 and the extra structure on the $\ell$-adic \'etale cohomology groups of~$X$
 (a Galois representation) convey the same information.

 The main result of this paper says that if $A_1$ and~$A_2$ are
 abelian varieties over~$K$, and the Mumford--Tate conjecture holds for
...
\begin{lemma} % {{{-2
 Let $T$ be a Tannakian category over a field~$Q$ of characteristic~$0$.
 Let $V$ be an object of~$T$, and $W$ and object of~$\Tangen{V}$.
 \begin{enumerate}
  \item We have $W^\ad \in \Tangen{V^\ad}$ and $W^\ha \in \Tangen{V^\ha}$.
  \item If $V$ is hyperadjoint, then $\Tangen{V}$ is semisimple.
  \item If $V$ and~$W$ are hyperadjoint, then $W$ is a direct summand of~$V$.
 \end{enumerate}
...
\begin{proposition} % {{{-2
 ...
 \begin{proof}
  ...
  and $a$ exists by \cref{centre-mtc-abelian-motive}.
  Moreover, $a$ is an isomorphism on connected components,
  also by \cref{centre-mtc-abelian-motive}.
  ...
  then $\Glc(M^\ha)$ is an adjoint group,
  and therefore the absolute rank of $\Glc(M)$
  is the sum of the absolute ranks of $\Glc(M^\ha)$ and~$\Zentrum(\Glc(M))$.
  Now we apply the Borel--de Siebenthal theorem
  (see~\cite{BdS} or~\cite{PL2015}):
  since $\Glc(M)$ has maximal rank as subgroup of $\GB(M) \otimes \QQl$,
  it is equal to the connected component of the centraliser of its centre.
  But since $a$ is an inclusion, we conclude that $b$ is an isomorphism.
  In other words: $\MTC_\ell(M) \iff \MTC_\ell(M^\ha)$.
 \end{proof}
\end{proposition}
...
\end{align*}
Let $G' \subset \GL_{E_\lambda}(\Hlambda)$ be the subgroup
...
\paragraph{} % {{{-2
We are almost done!
Recall that $T'$ is a finite quotient of~$(\Res_{E_\lambda/\QQl}G')^\ab$.
Hencethe group $\Gamma = \ker(\pi^\ad) \cap \ker(\pi^\ab)$ is
