...
\newcommand{\hypad}[1]{#1^{\textnormal{ha}}}
...
\section{A construction by Deligne} % {{{-1

\readme % {{{-2
We recall proposition~2.3.10 of~\cite{Del_ShimVar},
and prove some properties of this construction.

\begin{construction} % {{{-2
 Constructs an abelian variety from an adjoint Hodge structure.

 \emph{Input:}
 Let \((G,X)\) be as in \S2.1.1 of~\cite{Del_ShimVar},
 with \(G\) a \(\QQ\)-simple adjoint algebraic group
 of type \(A\),~\(B\), \(C\), \(D^{\RR}\), or~\(D^{\HQ}\).
 Let \(F\) be a quadratic extension of~\(E\),
 equipped with a \(T\) as in \S2.3.9 of~\cite{Del_ShimVar}.

 \emph{Output:}
 A diagram
 \[
  (G,X) \longleftarrow (G^{\sharp},X^{\sharp}) \longrightarrow (\CSp(V), \Sds),
 \]
 where the derived group \((G^{\sharp})^{\der}\) is simply connected
 if \(G^{\sharp}\) has type \(A\),~\(B\), \(C\), or~\(D^{\RR}\),
 and \((G^{\sharp})^{\der}\) is the covering of~\(G\)
 described in \S2.3.8 of~\cite{Del_ShimVar}
 if \(G^{\sharp}\) has type \(D^{\HQ}\).
 
 \emph{Construction:}
 See the proof of proposition~2.3.10 of~\cite{Del_ShimVar}.
\end{construction}

\section{An intermediate result} % {{{-1

\begin{theorem} \label{intermed} % {{{-2
 Let \(M_{1}\) and~\(M_{2}\) be two
...
\begin{enumerate}
 \item We first show that \(\End(M_{1}) = E = \End(M_{2})\),
  and \(\HH_{\Lambda}(M_{1}) = \HH_{\Lambda}(M_{2})\).
 \item We then run a construction by Deligne
  that produces two abelian varieties~\(A_{1}\) and~\(A_{2}\)
  such that \(M_{i} = \hypad{\HH(A_{i})}\).
 \item We show that \(\Hl^{1}(A_{1}) \cong \Hl^{1}(A_{2})\).
 \item By Faltings' theorem, we deduce \(\HH(A_{1}) \cong \HH(A_{2})\),
  which implies \(M_{1} \cong M_{2}\).
\end{enumerate}
