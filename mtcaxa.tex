 ...
 \begin{proof}
  ...
  \begin{enumerate}
   ...
   \item If $V$ is hyperadjoint, then $G$ is semisimple and thus reductive.
    ...
    follows from Goursat's lemma %TODO reference
    % maybe Cor 3 of LA 6.3 of Serre's "Lie Algebras and Lie groups" will work
...
\begin{example} % {{{-2
 ...
 whose underlying Dynkin diagrams have connected components of type~$D_4$.
 Let $\Delta^\mu$ be such an irreducible Deligne--Dynkin diagram
 over some field~$Q$.
 Recall that this means that we have the following:
 \begin{itemize}
  \item a diagram~$\Delta$ consisting of a number of copies of~$D_4$,
  \item an action of $\Gal(\bar Q/Q)$ on~$\Delta$
 ...
 the action of $\Gal(\bar Q/Q)$ and the subset $\mu \subset \Delta$
 play together:
 let $\bar\mu$ denote the closure of~$\mu$
 under the action of~$\Gal(\bar Q/Q)$.
 ...
 On the other hand, if $\Delta^\mu$ is populated ($\mu \ne \varnothing$),
 ...
 If $d = 1$, then $\deg(S) = 2$,
 and we say that $\Delta^\mu$ has type~$D_4^\RR$.
 If $d = 2$, then $\deg(S) = 1$,
 and we say that $\Delta^\mu$ has type~$D_4^\HQ$.
 We will not say more about these diagrams in this example,
 apart from mentioning that the distinction
 between diagrams of type~$D_4^\RR$ and~$D_4^\HQ$
...
Finally, if the connected components of~$\Delta$ have type~$D_4$,
...
We say that $\Delta^\mu$ has \emph{type}~$D_4^\RR$ (resp.~$D_4^\HQ$)
if $d = 1$ (resp.~$d = 2$).

We conclude with two observations:
\begin{itemize}
 \item The definitions of type~$D_n^\RR$ and~$D_n^\HQ$
  distinguish between the cases $n = 4$ and $n \ge 5$,
  but these cases unify in the following way:
  if $\Delta^\mu$ is of type~$D_n^\RR$ (resp.~$D_n^\HQ$),
 ...
 \item An irreducible symplectic populated Deligne--Dynkin diagram
  ...
  $A_n$~($n \ge 1$), $B_n$ ($n \ge 2$), $C_n$ ($n \ge 3$),
  $D_n^\RR$ ($n \ge 4$), or~$D_n^\HQ$ ($n \ge 4$);
  and all these types occur.
\end{itemize}

\begin{remark} % {{{-2
 \label{meaning-DnR-DnH}
 TODO %TODO explain the superscripts R and H.
\end{remark}
...
\begin{lemma} % {{{-2
 ...
 \begin{proof}
  ...
  If $\GdR(M)/K$ denotes the algebraic group $\Aut^\otimes(\HdR)$
  that corresponds to~$\Tangen{M}$,
  then $\gr\HdR$ corresponds to a cocharacter $\mu_\dR \in X_*(\GdR(M))$.

  The comparison theorem between the algebraic de~Rham realisation
  and the Betti realisation
  gives a natural transformation $\gr\HdR \otimes \CC \to \HB \otimes \CC$.
  This yields an isomorphism $\GdR(M) \otimes_K \CC \to \GB(M) \otimes_\QQ \CC$
  that maps $\mu_\dR$ to~$\mu$.

  On the other hand, $p$-adic Hodge theory compares
  the algebraic de~Rham realisation with the $p$-adic \'etale realisation,
  and gives a natural transformation
  $\gr\HdR \otimes \BHT{v} \to \Hp \otimes \BHT{v}$.
  This yields an isomorphism
  $\GdR(M) \otimes_K \CC \to \GB(M) \otimes_\QQ \BHT{v}$
  that maps $\mu_\dR$ to~$\mu_\HT$.
