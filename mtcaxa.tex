...
For any field~$K$,
we denote with~$\Gamma_K$ the absolute Galois group~$\Gal(\bar K/K)$ of~$K$.
In what follows,
we strive to denote ``ground fields'' with~$K$ or~$L$,
while we denote ``fields of coefficients'' with~$E$ or~$F$,
and occasionally~$Q$.
...
as follows:
Let $\bar\mu$ be the $\Gamma_Q$-closure of~$\mu$.
...
by $\Isom((\Delta_1,\mu_1),(\Delta_2,\mu_2))^{\Gamma_Q}$.

Observe that there is a natural map
$\Isom((\Delta_1,\mu_1),(\Delta_2,\mu_2))^{\Gamma_Q}
\to \Isom(\pi_0(\Delta_1),\pi_0(\Delta_2))^{\Gamma_Q}$,
and if $f \in \Isom(\pi_0(\Delta_1),\pi_0(\Delta_2))^{\Gamma_Q}$,
then we write $\Isom_f((\Delta_1,\mu_1),(\Delta_2,\mu_2))^{\Gamma_Q}$
for the set of
$\phi \in \Isom((\Delta_1,\mu_1),(\Delta_2,\mu_2))^{\Gamma_Q}$
...
$\phi \in \Isom_f((\Delta_1,\mu_1),(\Delta_2,\mu_2))^{\Gamma_\QQ}$.
...
\begin{proposition} % {{{-2
 ...
 $\psi_\ell \in
 \Isom_f((\Delta_1,\mu_1)_{\QQl},(\Delta_2,\mu_2)_{\QQl})^{\Gamma_{\QQl}}$.
 Then there exists a
 $\phi \in \Isom_f((\Delta_1,\mu_1),(\Delta_2,\mu_2))^{\Gamma_\QQ}$
...
%  \end{tikzpicture}
% \]
% Indeed, the map from $\Gamma_K$ to $\Gl(A)$
...
$\Gamma_K \to \Gl(A_i)(\QQl) \subset \GL(\Hl(A_i))(\QQl)$.
...
Define $\xi \colon \Gamma_K \to G'(E_\lambda)$
