...
\begin{definition} % {{{-2
 ...
 \begin{itemize}
  ...
  \item $\mu$ is a subset of~$\Delta$
   (that is not required to be $\Gal(\QQbar/\QQ)$-stable)
   such that
   \begin{enumerate*}[label=(\textit{\roman*})]
    \item $\mu$ meets every $\Gal(\QQbar/\QQ)$-component of~$\Delta$; and
    \item if $\mu$ meets a connected component of~$\Delta$,
...
\begin{definition} % {{{-2
 ...
 $\phi \colon (\Delta_1,\mu_1) \to (\Delta_2,\mu_2)$
 is a $\Gal(\QQbar/\QQ)$-equivariant isomorphism
 $\phi \colon \Delta_1 \to \Delta_2$ that maps $\mu_1$ onto~$\mu_2$.
\end{definition}

\begin{definition} % {{{-2
 \label{symplectic-nodes-deligne-dynkin-diagram}
 Let $(\Delta, \mu)$ be an Deligne--Dynkin diagram.
...
\begin{definition} % {{{-2
 A Deligne--Dynkin diagram $(\Delta,\mu)$ is \emph{symplectic}
 if for every $\Gal(\QQbar/\QQ)$-component of $(\Delta,\mu)$
...
with a subset of special nodes $\mu \subset \Delta$.
...
\paragraph{} % {{{-2
Let $(\Delta, \mu)$ be an irreducible symplectic Deligne--Dynkin diagram.
Let $\Aut_{\pi_0(\Delta)}(\Delta,\mu)$ be
the group of automorphisms of $(\Delta,\mu)$
that induce the identity on~$\pi_0(\Delta)$.
Then $\#\Aut_{\pi_0(\Delta)}(\Delta,\mu) \le 2$.
Indeed, if $(\Delta, \mu)$ has type $B_n$,~$C_n$ or~$D_n^\HQ$,
then $\#\Aut_{\pi_0(\Delta)}(\Delta,\mu) = 1$.
If $(\Delta, \mu)$ has type $D_n^\RR$,
then $\#\Aut_{\pi_0(\Delta)}(\Delta,\mu) = 2$.
If $(\Delta, \mu)$ has type $A_n$,
then there are two options:
the opposition involution is non-trivial (\emph{i.e.}, $n \ge 2$)
and fixes $\mu$, in which case $\#\Aut_{\pi_0(\Delta)}(\Delta,\mu) = 2$;
otherwise $\#\Aut_{\pi_0(\Delta)}(\Delta,\mu) = 1$.
...
\begin{proposition} % {{{-2
 ...
 \begin{proof}
  ...
  and $\Glambda(M_i) = \Glambdac(M_i)$.
  By \cref{todo} we also know that
  ...
  As in the statement, write $E$ for $E_1 = E_2$,
  ...
  For $i = 1,2$, let $E_i$ denote the endomorphism algebra of~$M_i$.
  By \cref{TODO}
  we find that
  \begin{enumerate*}[label=(\textit{\roman*})]
   \item there is a canonical isomorphism $E_1 \cong E_2$,
    so that we may write $E$ for $E_1 = E_2$; and
   \item there is a canonical isomorphism between the compatible systems
    $\HLambda(M_1)$ and~$\HLambda(M_2)$,
    where $\Lambda$ is the set of finite places of~$E$.
  \end{enumerate*}

  Let $I$ denote the set of real embeddings of~$E$.
  For $i = 1,2$, let $(\Delta_i, \mu_i)$
  be the Deligne--Dynkin diagram associated with~$M_i$,
  as in \cref{abmotdeldyn}.
  Fix a prime number~$p$.
  By \cref{TODO}
  we see that the isomorphism $\Hp(M_1) \cong \Hp(M_2)$
  TODO
  ...
  implies that $A_{1,L}$ and~$A_{2,L}$ are isogenous.
  Thus $M_{1,L} \cong M_{2,L}$,
  and the proof concludes by
  \cref{isomorphic-motives-field-extension}.
 \end{proof}
\end{proposition}
