...
%TODO This is ugly. Make sure that the first step is typeset
% on the same line as the paragraph head.
\label{strategy}
\begin{enumerate}
...
\begin{definition} % {{{-2
 ...
 for the algebraic group over~$E_\lambda$ that is
 the Zariski closure of the image of $\Gal(\bar K/K)$ under this representation.
...
\begin{remark} % {{{-2
 ...
 is an irreducible $\Gal(\bar Q/Q)$-subset $\Delta'$
...
The assumptions on $M$ imply that
...
\begin{lemma} % {{{-2
 ...
 \begin{proof}
  ...
  a non-trivial element of $\Aut_\id(\Delta,\mu)$
...
\begin{proposition} % {{{-2
 ...
 $\psi_\ell \in \Isom_f((\Delta_1,\mu_1)_{\QQl},(\Delta_2,\mu_2)_{\QQl})$.
...
\begin{lemma} % {{{-2
 ...
 \begin{proof}
  ...
  Recall that $\GG(M)/\QQ$ is the algebraic group $\Aut^\otimes(\HB)$
...
Note that $\Hom(E,\QQbar) \cong \pi_0(\Delta)$.
...
such that $\Gl(M_1 \oplus M_2) \subsetneq \Gl(M_1) \times \Gl(M_2)$.
By Goursat's lemma this implies that there are % TODO reference for Goursat
...
$\psi_\ell \in \Isom_f(\Delta_{1,\QQl},\Delta_{2,\QQl})$.
...
\begin{remark} % {{{-2
 We give a short list of examples of varieties
 ...
 See \S2.4,~\S3.3, and~\S4.4 of~\cite{Mo17} for more examples.
\end{remark}
...
% {{{-1 Footer
\printbibliography
%TODO Ser13 should by Ser81 or something like that
