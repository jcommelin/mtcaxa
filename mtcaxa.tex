...
An \emph{$R$-valued fibre functor}
is a $Q$-linear monoidal functor $T \to \Mod_R$
that is faithful and exact and takes values in the subcategory
$\Proj_R \subset \Mod_R$ of finitely generated projective $R$-modules.
...
a $Q$-algebra~$R$ and a fibre functor $T \to \Mod_R$.
...
The composite morphism $\mu_h = h \circ \mu_0$
is called the \emph{Hodge cocharacter} of~$V$.
If there is no confusion, we will usually write $\mu$ for~$\mu_h$.
...
By work of Serre~\cite{Serre_letter_to_Ribet}, there is a finite extension $L/K$
...
(see the discussion in~\S1.2.1 of~\cite{Del_ShimVar}),
